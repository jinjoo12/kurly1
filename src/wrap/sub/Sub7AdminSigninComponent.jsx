...
export default function Sub7AdminSigninComponent() {
    ...
    const onChangeSigninPw=(e)=>{
        setState({
            ...state,
            입력비밀번호:e.target.value
        })
    }
    ...
    const onSubmitSignin=(e)=>{
        ...
        formData.append('adminPw', state.입력비밀번호);

        axios({
            url:'https://ljj4771.com/kurly/green_kurly_admin_signin.php',
            method:'POST',
            data:formData
        })
        .then((res)=>{
            if(res.status===200){
                if(res.data!==''){
                    ...
                    localStorage.setItem('KURLY_SIGNIN_DATA', JSON.stringify(obj));
